...
function symbolNode(value, height, depth, italic, skew, classes, style) {
    ...
    if (/[Ã®Ã¯Ã­Ã¬]/.test(this.value)) {    // add Ä« when we add Extended Latin
...
defineSymbol(text, main, textord, "\u2013", "â€“");
defineSymbol(text, main, textord, "\u2014", "â€”");
defineSymbol(text, main, textord, "\u2018", "â€˜");
defineSymbol(text, main, textord, "\u2019", "â€™");
defineSymbol(text, main, textord, "\u201c", "â€œ");
defineSymbol(text, main, textord, "\u201d", "â€");
});
...
groupTypes.mod = function(group, options) {
    ...
    if (group.value.modType === "bmod") {
        // â€œ\nonscript\mskip-\medmuskip\mkern5muâ€
        if (!options.style.isTight()) {
...
/**
 * Given a pair of tokens (this and endToken), compute a â€œTokenâ€ encompassing
...
function punctuation(text){
  ...
  // var NBSP = "\u00a0";
  // var NBSP_PUNCTUATION_START = /([Â«Â¿Â¡]) /g;
  // var NBSP_PUNCTUATION_END = / ([\!\?:;\.,â€½Â»])/g;
...
function quotes(text) {
  ...
  text = text
    .replace(/(\W|^)"([^\s\!\?:;\.,â€½Â»])/g, '$1\u201c$2') // beginning "
